...
ui.helper.PrologHelper.termToLabel = function(term) {
	...
	if(term.name == "and") Lambda.iter(term.partlist.list,function(term1) {
		...
		if(term.name == "node") {
			...
			var progenyTerm = termParts.last();
...
ui.model.Label = function(text,term) {
	ui.model.ModelObj.call(this);
	...
	if(term == null) {
		...
		var termParts = term.partlist.list;
		var termPartsItr = termParts.iterator();
		var textTerm = termPartsItr.next();
		var textTermParts = textTerm.partlist.list;
		var textTermAtom = textTermParts.first().name;
		var displayTerm = termPartsItr.next();
		var displayTermParts = displayTerm.partlist.list;
		var displayTermPartsItr = displayTermParts.iterator();
		var colorTerm = displayTermPartsItr.next();
		var colorTermParts = colorTerm.partlist.list;
		var colorTermAtom = colorTermParts.first().name;
		var imageTerm = displayTermPartsItr.next();
		var imageTermParts = imageTerm.partlist.list;
		var imageTermAtom = imageTermParts.first().name;
